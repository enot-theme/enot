#!/usr/bin/env python3
"""Ranger colorscheme for the "enot" variant.

Writes ranger/colorschemes/enot.py. Colors are given as ANSI indices
0-15, the actual values come from the terminal palette
(wezterm/enot-*.toml), so one file serves both dark and light themes.
Install: copy into ~/.config/ranger/colorschemes/ and add
"set colorscheme enot" to rc.conf. Requires ranger >= 1.9.3; the
line_number key appeared after 1.9.3, so it is read via getattr.
"""

import os

# the class must be named Scheme: under any other name ranger may pick
# an imported base class from the module instead of ours
SCHEME = '''\
...
def main():
    os.makedirs("ranger/colorschemes", exist_ok=True)
    path = "ranger/colorschemes/enot.py"
