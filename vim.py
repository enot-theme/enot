#!/usr/bin/env python3
"""Vim colorscheme from colors.json (enot variant).

Writes vim/colors/enot.vim: both themes in one file (branching on
&background), guifg/guibg from the 16M depth of the spec, cterm from
the 256 depth, vim/nvim terminal palettes from ANSI 16. No colors are
computed here -- pure substitution from colors.json.
Install: copy into ~/.vim/colors/ or ~/.config/nvim/colors/.
"""
...
"""
    import os
    os.makedirs("vim/colors", exist_ok=True)
    path = f"vim/colors/{SLUG}.vim"
