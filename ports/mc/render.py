...
use the 256 depth of the spec. A terminal without truecolor does not
degrade a skin color by color: mc falls back to the default skin
entirely, so the 256 variant is mandatory. No colors are computed
here -- the alias and section tables below are the only mc-specific
mapping.
"""
...
def skin(roles, mode, depth):
    ...
    out = [f"# {SLUG} -- generated from colors.json by the enot build,"
           " do not edit by hand",
           "",
           "[skin]",
           f"description = {desc}",
